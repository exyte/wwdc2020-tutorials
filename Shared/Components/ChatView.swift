...
struct SupportChatView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            }.padding(15).navigationTitle("Chat").navigationBarItems(trailing: Button(action: {
                presentationMode.wrappedValue.dismiss()
            }) {
                HStack(spacing: 10) {
                    Image(systemName: "xmark")
                }.padding(10).background(Color.black.opacity(0.4)).clipShape(Circle()).accentColor(.white)
            })
        }
    }
